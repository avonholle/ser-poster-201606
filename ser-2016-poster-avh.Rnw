    ...
    % fill each column with content
        ...
        \begin{block}{Introduction}
          ...
          \small{
            ...
            \item[\colouredcircle] Z-scores are a frequent outcome choice when modeling weight growth trajectories. \textbf{Advantages} include sex- and age- independent dimensionless measures. \textbf{Disadvantage} is measure designed for cross-sectional use.
...
\noindent\rule{20cm}{0.4pt}
        ...
        \item[\colouredcircle] After simulating weight values, we converted simulated weight to Z-score and percentiles and fit three models:
          \item[\colouredcircle] \textbf{Model 1}: $E(m_{ij}) = \beta_0 + \beta_1 \cdot \texttt{t} + \beta_2 \cdot \texttt{group} + \beta_3 \cdot \texttt{t} \cdot \texttt{group}$
%Linear regression with first order term for time: 

          \item[\colouredcircle] \textbf{Model 2} $E(m_{ij}) = \beta_0 + \beta_1 \cdot \texttt{t} + \beta_2 \cdot \texttt{group} + \beta_3 \cdot \texttt{t} \cdot \texttt{group} + \beta_4 \cdot \texttt{t}^2$
%Linear regression with second order term for time: 

          \item[\colouredcircle] \textbf{Model 3} \small $E(m_{ij}) = \beta_0 + \beta_1 \cdot \texttt{month6} + \beta_2 \cdot \texttt{group} + \beta_3 \cdot \texttt{month6} \cdot \texttt{group}$
...
\begin{beamercolorbox}[center,wd=\textwidth]{postercolumn}
        ...
        \begin{block}{Conclusions}
          ...
          \begin{itemize}[align=left, labelindent=1pt,leftmargin=!]
            ...
            \item[\colouredcircle] Even at similar age and gender values, reference to a standard, WHO Z-scores, is not equivalent to crude weight values when assessing group differences in weight change over time.             \item[\colouredcircle] Supporting this conclusion, estimated group differences in weight change for all three outcomes only coincide in direction under certain conditions -- equivalent weight at baseline.
              \begin{itemize}[align=left, labelindent=2pt,leftmargin=!]
                \item[$\Rightarrow$] Special attention needed when interpreting standardized measures in longitudinal models. What is your target population?
              \end{itemize}
              
          \item[\colouredcircle] If comparisons to a standard such as WHO are not an objective, using Z-scores instead of crude weight can lead to
            \begin{itemize}[align=left, labelindent=2pt,leftmargin=!]
              \item[$\Rightarrow$] biased estimates and conflicting inference.
              \item[$\Rightarrow$] lower power and inflated type I error.
              \end{itemize}
              
            \item[\colouredcircle] Findings replicate lower power found in adolescent age group \cite{berkey_adiposity_2007}. 
